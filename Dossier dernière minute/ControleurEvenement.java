...
public class ControleurEvenement {
	...
	public ControleurEvenement() {
		...
		//this.enregistrerEvenementsFichier(); Remet le fichier � 0;
		//this.chargerEvenementFichier();
		identifiant = 0;//listeEvent.get(listeEvent.size()-1).getIdEvenement() + 1; //on r�cup�re l'id du dernier evenement et on lui ajoute 1 comme �a on est sur de ne jamais avoir deux id identiques.
	...
	/**
	 * Démarre les controleur evenement<br>
	 * @return void
	 */
	...
	/**
	 * Permet d'ajouter un evenement à la liste des événements
	 * @param _e Evenement
	 * @return void
	 */
	...
	/**
	 * Permet de supprimer un evenements situé a un rang donné (dans un vector)
	 * @param _index
	 * @return void
	 */
	public void supprimerEvenement(int _index){
		if ( _index == -1){
			JOptionPane.showMessageDialog(lambda, "Pas de question s�lectionn�e!");
		}
		else{
			int rep = JOptionPane.showConfirmDialog(lambda, "Etes-vous sur de vouloir supprimer l'�v�nement?","Supprimer",JOptionPane.YES_NO_OPTION);
	...
	public void ajouterEvenement (){
		...
		this.det.setLocationRelativeTo(lambda); //fenetre centr�e
